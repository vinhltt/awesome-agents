...
# =============================================================================
# Usage: bash .specify/scripts/bash/sync-commands.sh [--dry-run]
#
# Source: .specify/commands/
# Targets:
#   - .claude/commands/           (speckit.*.md, ut/*.md)
#   - .github/prompts/            (speckit.*.prompt.md, ut.*.prompt.md)
#   - .gemini/commands/           (speckit.*.toml - converted)
...
# Looks for the main command header (e.g., "# /speckit.plan - Description")
extract_description() {
  local file="$1"
  local name
...
sync_speckit_commands() {
  log_info "Syncing SpecKit commands..."

  local source_speckit="$SOURCE_DIR/speckit"

  if [[ ! -d "$source_speckit" ]]; then
    log_error "Source directory not found: $source_speckit"
    return 1
  fi
  ...
  local count=0
  for file in "$source_speckit"/*.md; do
    [[ ! -f "$file" ]] && continue

    local name=$(basename "$file" .md)

    # Claude: speckit.{name}.md
    copy_file "$file" "$CLAUDE_DIR/speckit.$name.md"

    # GitHub Copilot: speckit.{name}.prompt.md
    copy_file "$file" "$GITHUB_DIR/speckit.$name.prompt.md"

    # OpenCode: speckit.{name}.md
    copy_file "$file" "$OPENCODE_DIR/speckit.$name.md"

    # Gemini: speckit.{name}.toml (converted)
    # Skip status command for Gemini (not supported)
    if [[ "$name" != "status" ]]; then
      convert_to_toml "$file" "$GEMINI_DIR/speckit.$name.toml"
    fi

    count=$((count + 1))
  done

  log_success "SpecKit: $count commands synced"
}
