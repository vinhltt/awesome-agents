...
while [ $i -le $# ]; do
    ...
    case "$arg" in
        ...
        --help|-h)
            ...
            echo ""
            echo "Configuration:"
            echo "  Allowed prefixes: $SPECKIT_PREFIX_LIST"
            echo "  Default folder: $SPECKIT_DEFAULT_FOLDER"
            echo "  Main branch: $SPECKIT_MAIN_BRANCH"
            echo ""
            echo "Examples:"
...
if [ "$folder" = "$SPECKIT_DEFAULT_FOLDER" ]; then
...
SPECS_DIR="$REPO_ROOT/$SPECKIT_SPECS_ROOT/$folder"
mkdir -p "$SPECS_DIR"
...
check_ticket_exists() {
    ...
    if [ -d "$REPO_ROOT/$SPECKIT_SPECS_ROOT/$folder/$ticket_id" ]; then
        return 0  # Exists
    fi
...
TEMPLATE="$REPO_ROOT/$SPECKIT_SPECS_ROOT/templates/spec-template.md"
