...
3. **Infer from conversation context**:
   ...
   **If NO context found**:
   ...
   Usage: /speckit.implement <task-id>
   Example: /speckit.implement pref-001
...
4. **Validate task_id format**:
   ...
   - Examples:
     - ✅ `/speckit.implement pref-001` → task_id: `pref-001`
     - ✅ `/speckit.implement PREF-001` → task_id: `pref-001` (case-insensitive)
     - ✅ `/speckit.implement hotfix/pref-123` → task_id: `hotfix/pref-123`
     - ❌ `/speckit.implement` without context → ERROR (no task ID)
...
Run `.specify/scripts/bash/check-prerequisites.sh {task_id} --json --require-tasks --include-tasks` from repo root (pass the validated task_id from Step 0). Parse JSON for TASK_ID, FEATURE_DIR, AVAILABLE_DOCS.
