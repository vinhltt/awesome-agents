...
3. **Infer from conversation context**:
   ...
   **If NO context found**:
   ...
   Usage: /ut.plan <task-id>
   Example: /ut.plan pref-001
...
4. **Validate task_id format**:
   ...
   - Examples:
     - ✅ `/ut.plan pref-001` → task_id: `pref-001`
     - ✅ `/ut.plan PREF-001` → task_id: `pref-001` (case-insensitive)
     - ❌ `/ut.plan` without context → ERROR (no task ID)
