...
3. **Validate task ID format**:
   ...
   - Examples (assuming prefix=pref):
     - ✅ `/speckit.implement pref-001` → feature ID: `pref-001`
     - ✅ `/speckit.implement hotfix/pref-123` → feature ID: `hotfix/pref-123`
     - ✅ `/speckit.implement AL-991` → feature ID: `AL-991` (if AL in prefix list)
...
Run `.specify/scripts/bash/check-prerequisites.sh --json --require-tasks --include-tasks` from repo root (NO task ID argument) and parse FEATURE_DIR and AVAILABLE_DOCS list.
