...
- **SpecKit Default Workflow**: spec.md → plan.md → tasks.md → implementation
- **UT Workflow**: test-spec.md → coverage-report.json → test-plan.md → generated tests → review → execution
- **Mixed State**: Features using both workflows
...
3. **Validate task ID format**:
   ...
   - Examples (assuming prefix=pref):
     - ✅ `/speckit.status {prefix}-001` → e.g., `pref-001`
     - ✅ `/speckit.status hotfix/{prefix}-123` → e.g., `hotfix/pref-123`
     - ✅ `/speckit.status AL-991` → if AL in SPECKIT_PREFIX_LIST
...
FEATURE_DIR=".specify/features/{feature-id}"

if not exists:
  Report: "Feature {feature-id} not found. Run: /speckit.specify {feature-id}"
  Exit
```
...
**SpecKit Default Workflow Artifacts**:
...
2. **SpecKit Only**: Has spec.md/plan.md/tasks.md
...
### Step 3: Analyze SpecKit Default Workflow
...
git branch --list "features/{feature-id}" "feature/{feature-id}"

# Check uncommitted changes
git status --porcelain .specify/features/{feature-id}
```
...
2. **Detect Workflow Type**: SpecKit only, UT only, or both
...
After this:
  → /speckit.plan {feature-id} (create implementation plan)
  → /ut.specify {feature-id} (TDD: write tests first)
...
After this:
  → /speckit.implement {feature-id} (start coding)
  → /ut.specify {feature-id} (write tests first)
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  Workflow:
    1. /speckit.implement → code implementation
    2. /ut.specify → define tests
...
After completing current phase:
  ...
  → /ut.specify {feature-id} (add tests if not done)
...
After this:
  ...
  → /speckit.implement {feature-id} (continue implementation)
```
...
Failure Analysis:
  ...
  📝 Details: .specify/features/{feature-id}/test-results.md
...
After fixing:
  ...
  → /speckit.implement {feature-id} (when all pass)
{endif}
...
Workflow Status:
  📋 SpecKit: {speckit-phase} ({speckit-percent}% complete)
...
Alternative: Focus on one workflow
  • Complete SpecKit first, then comprehensive testing
...
1. **Workflow Balance Score**:
   ```
   speckit_progress = (completed_phases / total_phases) * 100
   ut_progress = (completed_steps / 6) * 100
   balance_score = abs(speckit_progress - ut_progress)
...
💡 TIP: Workflow Imbalance Detected
  ...
  SpecKit Progress: 85% (Implementation nearly done)
...
If no feature ID provided, list all features:
...
Use: /speckit.status <feature-id> for details
```
...
- [ ] Both workflows (SpecKit + UT) detected correctly
...
- **Feature not found**: List available features, suggest creation
...
- Supports **both workflows** - SpecKit default and UT
