...
3. **Infer from conversation context** (only if task_id missing):
   ...
   **If NO context found**:
   ...
   Usage: /ut.review <task-id>
   Example: /ut.review pref-001
...
4. **Validate task_id format**:
   ...
   - Examples:
     - ✅ `/ut.review pref-991` → task_id: `pref-991`
     - ✅ `/ut.review PREF-991` → task_id: `pref-991` (case-insensitive)
     - ❌ `/ut.review` without context → ERROR (no task ID)
