...
3. **Infer from conversation context** (only if task_id missing):
   ...
   **If NO context found**:
   ...
   Usage: /ut.analyze <task-id>
   Example: /ut.analyze pref-001
...
4. **Validate task_id format**:
   ...
   - Examples:
     - ✅ `/ut.analyze pref-991` → task_id: `pref-991`
     - ✅ `/ut.analyze PREF-991` → task_id: `pref-991` (case-insensitive)
     - ✅ `/ut.analyze test/pref-123` → task_id: `test/pref-123`
     - ❌ `/ut.analyze` without context → ERROR (no task ID)
     - ❌ `/ut.analyze create analysis` → ERROR (not a valid task ID)
