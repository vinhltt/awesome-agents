...
Hướng dẫn sử dụng bộ commands UT (Unit Test) trong `.specify` framework.
...
**Yêu cầu trước**:
- Phải có `spec.md` trong `.specify/features/{feature-id}/`
- Chạy `/speckit.specify {feature-id}` nếu chưa có
...
**Output** (trong `.specify/features/{feature-id}/`):
...
Files created:
  - .specify/features/pref-001/test-spec.md
  - .specify/features/pref-001/coverage-analysis.md
  - .specify/features/pref-001/test-plan.md
...
# 2. Với Python features, AI tự detect
